...
export async function GET(req: NextRequest) {
  try {
    ...
    const url =
      `${getBaseUrl()}/v1/snapshot?symbols=${encodeURIComponent(symbols.join(","))}&fiat=${encodeURIComponent(fiat)}`;
    ...
    return NextResponse.json({ ok: false, error: e?.message ?? "snapshot_error" }, { status: 500 });
  }
}
