...
export async function GET(req: NextRequest) {
  try {
    ...
    const url =
      `${getBaseUrl()}/v1/risk-flags?symbols=${encodeURIComponent(symbols.join(","))}&fiat=${encodeURIComponent(fiat)}`;
    ...
    return NextResponse.json({ ok: false, error: e?.message ?? "risk_flags_error" }, { status: 500 });
  }
}
