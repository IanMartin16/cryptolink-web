...
export async function GET(req: NextRequest) {
  try {
    ...
    const url =
      `${getBaseUrl()}/v1/prices/spark?symbols=${encodeURIComponent(symbols.join(","))}&fiat=${encodeURIComponent(fiat)}`;

    const apiKey = process.env.CRYPTOLINK_DEMO_KEY || "";
    const res = await fetch(url, {
      // spark cambia poco (buffer/BD); revalidate corto pero > que precio en vivo
      next: { revalidate: 30 },
    ...
    return NextResponse.json({ ok: false, error: e?.message ?? "spark_error" }, { status: 500 });
  }
}